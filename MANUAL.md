# 🚀 HDAT-DA 인증시험 최종 마스터 가이드

이 문서 하나면 어떠한 시험 환경과 파일이 주어지더라도 완벽하게 대처할 수 있습니다. 

---

## 📁 1. 폴더 및 파일 구성 가이드

현재 폴더에는 다음과 같은 파일들이 준비되어 있습니다.

*   **`study_tutor.py` (대화형 인터랙티브 학습 튜터 🎓)**
    *   **용도**: DataFrame의 변형 과정과 분류/회귀 원리를 단계별로 대화하며 직접 눈으로 공부하는 과외 프로그램입니다.
    *   **실행**: `python study_tutor.py`
*   **`universal_exam.py` (실전용 궁극의 만능 무기 ⭐)**
    *   **용도**: 시험장이나 어떤 연습문제 환경에서도 데이터셋 구조를 자동 파악하여 실행되는 만능 마스터 코드입니다.
    *   **특징**: 타깃(Target), 문제 유형(회귀/분류), 평가지표, 누수(Leakage), 압축파일(ZIP) 해제 여부 등을 모두 스스로 판별하여 100% 양식 일치 제출 파일을 생성합니다.
*   **`MANUAL.md` (이 문서)**

---

## 💻 2. 외워야 할 궁극의 만능 마스터 코드 (어떤 포맷이든 무결점 방어)

> **단일 데이터셋만 주어지거나 `sample-submission.csv`가 없는 극한의 실전 환경에서도 스스로 판단하여 분할/생성해내는 7단계 오토 스크립트입니다.**
