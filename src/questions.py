...
def gen_easy_read():
    ...
    return {
        'topic': '데이터 로드', 'question': f"data.{ext} 파일을 읽어 df에 할당하세요.", 
...
def gen_easy_head():
    ...
    return {
        'topic': '데이터 미리보기', 'question': f"df의 상단 {n}개 행을 확인하세요.", 
...
def gen_easy_info():
    ...
    return {
        'topic': '데이터 메타정보', 'question': "df의 행 개수, 컬럼 타입, 결측치를 요약 출력하세요.", 
...
def gen_easy_isnull():
    ...
    return {
        'topic': '결측치 집계', 'question': "df의 각 컬럼별 결측치(NaN) 총합을 구하세요.", 
...
def gen_easy_drop():
    ...
    return {
        'topic': '컬럼 제거', 'question': f"df에서 {col} 컬럼을 삭제하세요.", 
...
def gen_easy_filter():
    ...
    return {
        'topic': '조건부 필터링', 'question': f"df에서 {col} 값이 {val} 이상(>=)인 행만 추출하세요.", 
...
def gen_hard_merge():
    ...
    return {
        'topic': '데이터 병합', 'question': f"df1과 df2를 'user_id' 기준으로 {how} Join 하세요.", 
...
def gen_hard_pivot():
    ...
    return {
        'topic': '피벗 테이블', 'question': f"df에서 행 '{idx}', 열 'month', 값 'sales', 집계 'sum'인 피벗 테이블 코드를 작성하세요.", 
